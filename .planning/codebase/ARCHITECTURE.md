...
- **Responsive UI**: Dashboard with stateful data fetching and manual trigger options
- **Persistent Storage**: SQLite database with ORM for all collected data and automation state
...
2. **Scraper** fetches data from external source (YouTube API, RSS, GitHub API, Apify)
3. **Scraper** calculates metrics (quality score for videos, trending score for projects)
4. **Scraper** stores data in database via SQLAlchemy ORM
5. **API endpoint** (`/api/videos`, `/api/news`, `/api/projects`) queries database
6. **Frontend** fetches via `lib/api.ts` and displays with filters/sorting
7. **User** views data, can trigger manual scrape to refresh
...
- **Frontend State**: React `useState` for UI state (loading, errors, form inputs)
- **Backend State**: Database records (YouTubeAutomation with status field) track pipeline progress
- **Async Operations**: AsyncIO scheduler manages background tasks, async/await for API calls
- **No Real-time Updates**: Frontend polls via manual refresh or fetch calls; no WebSocket
...
- **Scrapers**: Log errors during fetch, log to ScraperLog table with error_message field, continue to next scraper
