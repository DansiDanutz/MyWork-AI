...
export function ProfileForm({ user }: ProfileFormProps) {
  ...
  const saveName = useCallback<(value: string) => Promise<void>>(
    async (value: string) => {
      setStatus("saving");
      setError(null);

      const result: ProfileUpdateResult = await updateProfileField(
        "name",
        value,
      );
  ...
  const saveBio = useCallback<(value: string) => Promise<void>>(
    async (value: string) => {
      setStatus("saving");
      setError(null);
  ...
  const debouncedSaveName = useDebounce<(value: string) => Promise<void>>(
    saveName,
    3000,
  );
  const debouncedSaveBio = useDebounce<(value: string) => Promise<void>>(
    saveBio,
    3000,
  );
