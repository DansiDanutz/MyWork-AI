...
export async function POST(request: NextRequest) {
  try {
    ...
    const status = threshold
      ? metric.value <= threshold.good ? 'GOOD' : metric.value <= threshold.poor ? 'NEEDS_IMPROVEMENT' : 'POOR'
      : 'UNKNOWN'

    console.log(`[Vitals] ${metric.name}: ${metric.value.toFixed(2)} (${status}) - ${metric.navigationType}`)
