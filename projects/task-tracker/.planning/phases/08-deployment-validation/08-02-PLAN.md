...
<task type="auto">
  ...
  <action>
Create a Server Action to store user feedback using the existing analytics infrastructure:
...
3. Create submitFeedback action that:
   - Accepts: feedback text (string), type ('bug' | 'idea' | 'other'), page (string)
...
Note: REUSE the existing trackEvent function from @/shared/lib/analytics - do NOT create new database calls.
...
Contains: trackEvent({ type: 'feedback_submitted'
  ...
  <done>Feedback Server Action stores feedback via trackEvent from @/shared/lib/analytics</done>
</task>
