...
- from: "src/shared/actions/feedback.ts"

  to: "@/shared/lib/analytics"
  via: "trackEvent call"
  pattern: "trackEvent"
...
Purpose: Enable users to share their experience directly within the app,
providing framework validation data without requiring external tools.

Output: Floating feedback button, feedback form component, server action to
store feedback.
</objective>
...
<task type="auto">
  <name>Task 1: Create feedback Server Action with analytics tracking</name>
  <files>src/shared/actions/feedback.ts</files>
  <action>
Create a Server Action to store user feedback using the existing analytics
infrastructure:
...
3. Create submitFeedback action that:
   - Accepts: feedback text (string), type ('bug' | 'idea' | 'other'), page
...
```
   ...
   - Calls trackEvent to store feedback (REUSE existing Phase 6 pattern):
...
4. Validate input:
   - Feedback text: required, max 500 characters
   - Type: must be 'bug' | 'idea' | 'other'
   - Page: required string

Note: REUSE the existing trackEvent function from @/shared/lib/analytics - do
NOT create new database calls.
Feedback is stored as an analytics event with type 'feedback_submitted'.
  </action>
  <verify>
Check file exists: ls src/shared/actions/feedback.ts
Contains 'use server' directive
Contains submitFeedback export
Contains: import { trackEvent } from '@/shared/lib/analytics'
Contains: trackEvent({ type: 'feedback_submitted'
  </verify>
  <done>Feedback Server Action stores feedback via trackEvent from
  @/shared/lib/analytics</done>
</task>

<task type="auto">
  <name>Task 2: Create FeedbackWidget component</name>
  <files>src/shared/components/FeedbackWidget.tsx,
...
2. Add FeedbackWidget inside the body, after the main content but before closing

body tag

3. Position it so it doesn't interfere with the main layout

Note: FeedbackWidget handles its own positioning (fixed), so just include it as
a sibling to main content.
...
Submit feedback - should show success
  ...
  <done>FeedbackWidget integrated into root layout and accessible on all
  pages</done>
</task>
...
- Feedback is stored via trackEvent for later review in analytics_events

</success_criteria>

<output>
After completion, create
`.planning/phases/08-deployment-validation/08-02-SUMMARY.md`
</output>
