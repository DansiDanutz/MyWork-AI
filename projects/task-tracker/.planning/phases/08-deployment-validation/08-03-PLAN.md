...
Purpose: Establish automated deployment workflow and get the application live on
a public URL for real user testing.

Output: GitHub Actions workflow, Vercel configuration, deployed production
application.
</objective>
...
- Set maxDuration to 30 seconds for API routes (file uploads, database
...
id: deploy
run: |
  DEPLOY_URL=$(vercel deploy --prebuilt --prod --token=${{
  secrets.VERCEL_TOKEN }})
...
YAML is valid: python3 -c "import yaml;
...
Go to: GitHub Repository -> Settings -> Secrets and variables -> Actions -> New
repository secret
...
4. Note the deployment URL from the workflow output (e.g.,

<https://task-tracker-xxx.vercel.app>)
...
`.planning/phases/08-deployment-validation/08-03-SUMMARY.md`
</output>
