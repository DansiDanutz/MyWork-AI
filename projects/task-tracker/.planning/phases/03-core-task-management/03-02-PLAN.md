...
Purpose: Create reusable components that will be used across task pages -
TaskCard for individual tasks, TaskList for the grouped display, and TaskForm
for creating new tasks.

Output: Three client components with optimistic UI, status management, and
...
Component exports TaskCard function
  ...
  <done>TaskCard component displays task info with optimistic status updates and
...
Component properly groups tasks by status
  ...
  <done>TaskList component displays tasks in status-grouped sections with empty
  state handling</done>
</task>
...
Note: The form submits via Server Action which handles redirect. No client-side
routing needed.
  </action>
  <verify>
...
Form uses useActionState correctly
  ...
  <done>TaskForm component handles task creation with validation feedback and
  loading states</done>
</task>
...
`.planning/phases/03-core-task-management/03-02-SUMMARY.md`
</output>
