...
handlers
rationale: Simplifies auth flow, keeps credentials server-side, works
without JavaScript
...
**One-liner:** Complete authentication UI with GitHub OAuth login page, homepage
hero CTA, and post-OAuth welcome/onboarding flow.
...
**Key pattern:** Server Actions for authentication keep sensitive operations
server-side and work without JavaScript.
...
**Key pattern:** Conditional UI rendering based on session state provides
...
**Key pattern:** Post-OAuth onboarding flow helps new users understand app
