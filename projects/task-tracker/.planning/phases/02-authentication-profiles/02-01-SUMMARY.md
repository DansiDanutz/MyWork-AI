...
**Auth.js v5 with GitHub OAuth, database-backed sessions, and Prisma adapter for secure authentication**
...
2. **Task 2: Install Auth.js Dependencies and Create Configuration** - `3cdbb93` (feat)
...
- `src/app/api/auth/[...nextauth]/route.ts` - Catch-all route handler for all Auth.js endpoints
- `src/shared/types/next-auth.d.ts` - TypeScript declaration extending Session with user.id
...
- **First-time user flow:** Redirect to /welcome for onboarding instead of default page
- **Text column for tokens:** Used @db.Text for GitHub token fields to handle long tokens that exceed varchar(191) limit
...
- **Note:** This is a local development environment quirk, not a code issue. Documented for awareness.

## User Setup Required

**External services require manual configuration.** User must complete before authentication will work:
