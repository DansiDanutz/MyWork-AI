...
**Primary recommendation:** Use Server Actions for initial upload UI, but implement dedicated chunked upload API routes using TUS protocol or custom chunking for files > 5MB. This hybrid approach balances simplicity for small files with robustness for large files.
...
| file-type | 19.x | Content-based MIME detection | Security-critical, validates actual file content not just extension |
| sharp | 0.33.x | Image processing/thumbnails | Fastest Node.js image library (30x faster than jimp), libvips-based |
...
| tus-node-server | 1.x | Resumable uploads (server) | Files > 5MB needing resume capability |
| pompelmi | Latest | In-process malware scanning | Privacy-compliant virus scanning without cloud APIs |
| clamscan | 2.x | ClamAV Node.js wrapper | If ClamAV already in infrastructure |
...
**What:** Use Next.js Server Actions directly for simple file uploads without separate API routes
...
**What:** Implement TUS resumable upload protocol for chunked uploads with resume capability
...
**What:** Validate files by reading magic numbers (binary signatures), not extensions
...
**What:** Serve files through authenticated route handlers, not direct filesystem access
...
- **❌ Trusting client-provided MIME types:** Always validate with file-type (reads magic numbers)
- **❌ Storing files in public/ directory:** Files become publicly accessible without auth
...
- **❌ Missing AbortController for upload cancellation:** Can't stop in-progress uploads
- **❌ Server Actions for large files (> 5MB):** Hits body size limit, no progress tracking
...
|---------|-------------|-------------|-----|
| Chunked uploads with resume | Custom chunk logic | TUS protocol (tus-js-client + tus-node-server) | Handles chunk ordering, resume tokens, partial chunk recovery, standard protocol |
| MIME type detection | RegEx on file extension | file-type (npm) | Reads magic numbers, detects spoofed extensions, validates actual content |
| Image thumbnail generation | Canvas API or custom resizing | Sharp | 30x faster, handles EXIF rotation, memory-efficient streaming, supports all formats |
| Drag & drop UI | Native HTML5 API | react-dropzone | Accessibility built-in, file validation, multi-file handling, keyboard navigation |
| File path traversal prevention | String manipulation | path.join() + validation | Prevents ../../../etc/passwd attacks, normalizes paths |
| Virus scanning | Custom signature detection | ClamAV (clamscan) or pompelmi | 8M+ malware signatures, regular updates, battle-tested |
| Upload progress tracking | Manual percentage calculation | XMLHttpRequest.upload.onprogress or TUS | Handles network variability, partial uploads, browser differences |
| Concurrent upload limiting | Custom queue | p-limit (npm) | Handles backpressure, errors, cancellation |

**Key insight:** File upload security is complex—path traversal, malware, MIME spoofing, zip bombs, executable disguised as images. Use battle-tested libraries, not custom validation logic.

## Common Pitfalls

### Pitfall 1: Server Actions Body Size Limit on Vercel

**What goes wrong:** Files > 1MB (default) or > 5MB (configured) fail with 413 error on Vercel, even though they work locally
...
**What goes wrong:** User uploads malicious.exe renamed to photo.jpg, server accepts it based on extension or client-provided Content-Type, malware reaches server
...
**What goes wrong:** User uploads file named `../../../etc/passwd`, server writes to attacker-controlled location, overwrites critical files or reads sensitive data
...
**What goes wrong:** Tasks deleted but files remain on disk, wasting storage. Orphaned files grow unbounded.
...
**What goes wrong:** Server becomes unresponsive during file uploads, other requests time out, poor concurrency
...
**What goes wrong:** Users can't stop in-progress uploads, wasted bandwidth, poor UX when user realizes they selected wrong file
...
**What goes wrong:** All uploaded files become publicly accessible without authentication, anyone can access any file by guessing URLs
...
|--------------|------------------|--------------|--------|
| Multer middleware + API routes | Server Actions with FormData | Next.js 13 (2022) | Simpler code, type-safe, no manual API routes for small files |
| jimp for image processing | Sharp with libvips | Sharp 0.30+ (2022) | 30x faster, WebP/AVIF support, better memory usage |
| Custom chunking logic | TUS protocol | TUS 1.0 (2020) | Standard protocol, interoperable clients/servers, resume tokens |
| Extension-based validation | Magic number validation (file-type) | Security best practice | Prevents MIME type spoofing attacks |
| react-beautiful-dnd for file drag & drop | react-dropzone | N/A (different use cases) | react-dropzone designed for files, not sortable lists |
| Blacklist dangerous extensions | Whitelist allowed MIME types | OWASP guidelines | More secure (can't forget obscure dangerous types) |
...
1. **Virus scanning approach**
   - What we know: ClamAV is standard, pompelmi offers in-process scanning, cloud APIs exist (Verisys)
   - What's unclear: Best practice for this scale (single user task tracker vs multi-tenant SaaS)
   - Recommendation: Start without virus scanning (out of scope for v1), add pompelmi if needed in v2. Context specifies "security scanning" but single-user task tracker has lower risk than public file hosting.

2. **Chunked upload strategy**
   - What we know: Context requires 25MB files + resume capability, Server Actions max out ~5MB
   - What's unclear: Whether to implement TUS protocol (complex but standard) or custom chunking (simpler but more code)
   - Recommendation: Use TUS protocol via tus-js-client + @tus/server. It's battle-tested and handles edge cases we'd miss in custom implementation.

3. **Thumbnail generation timing**
   - What we know: Sharp is fast but synchronous operations block event loop
   - What's unclear: Generate thumbnails synchronously on upload or queue for background processing
   - Recommendation: Synchronous for phase 5 (simpler), move to background queue if performance issues arise. Sharp is fast enough (<500ms for most images).

4. **Storage scalability**
   - What we know: Local filesystem chosen in CONTEXT.md
   - What's unclear: File size limits over time, backup strategy, what happens at 10GB+ of files
   - Recommendation: Local filesystem is fine for v1. If storage becomes issue, migrate to S3-compatible storage in v2 (Prisma schema already tracks paths, easy to migrate).
...
- [TUS Protocol Specification](https://tus.io/protocols/resumable-upload) - Official resumable upload protocol
...
**Valid until:** 2026-02-25 (30 days - Next.js ecosystem stable, patterns established)
