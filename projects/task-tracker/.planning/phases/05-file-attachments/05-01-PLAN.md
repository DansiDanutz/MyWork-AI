...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/shared/lib/file-validation.ts"
      ...
      exports: ["validateFileType", "validateFileSize", "ALLOWED_MIME_TYPES", "MAX_FILE_SIZE"]
  key_links:
...
<objective>
Create the database schema for file attachments and core file validation utilities.

Purpose: Establish the data model and security foundation for file uploads. Content-based MIME validation is critical for security - we must validate actual file content, not just extensions.

Output: FileAttachment Prisma model, file-validation.ts utility, updated next.config.ts with body size limit
</objective>
...
Note: Files larger than 5MB will use TUS protocol via dedicated API routes.
  ...
  <verify>
Run `npm ls react-dropzone file-type sharp tus-js-client` to confirm all packages installed.
...
Run `npx prisma studio` and verify FileAttachment table exists with correct columns.
