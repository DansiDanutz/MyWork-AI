...
<objective>
Implement thumbnail generation, Server Actions for file operations, and secure download endpoint.

Purpose: Server Actions handle small file uploads (< 5MB) directly without TUS complexity. The download endpoint ensures files are served with proper authentication and ownership checks.

Output: thumbnail-generator.ts, files.ts Server Actions, download API route, DAL updates
</objective>
...
<task type="auto">
  ...
  <action>
Replace the placeholder with full implementation in src/shared/lib/thumbnail-generator.ts:
...
Add file-related DAL functions to src/shared/lib/dal.ts (append to existing file):
