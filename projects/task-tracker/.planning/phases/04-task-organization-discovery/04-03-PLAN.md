...
Purpose: Enable users to organize tasks with tags during creation and editing, displaying tags visually on task cards with color-coded badges.

Output: TagInput component with autocomplete, TagBadge display component, updated TaskCard/TaskForm/TaskEditForm with tag support.
</objective>
...
```
  ...
  <done>TagBadge component exists with color support, optional remove button, and size variants</done>
</task>
...
```
  ...
  <verify>TypeScript compiles: `npx tsc --noEmit`</verify>
  <done>TagInput component with autocomplete dropdown, create-on-enter, and loading states</done>
</task>
...
<task type="auto">
  ...
  <action>
Update TaskCard to show tags on task cards. Modify the existing TaskCard component:
...
Place this JSX after the title `<h3>` element and before the description `<p>` element.
...
Also update the useOptimistic to handle tags in the task object.
  ...
  <done>TaskCard displays tags with color-coded badges, showing max 3 with "+N more" overflow</done>
</task>

<task type="auto">
  <name>Task 4: Update TaskForm to include tag input</name>
  <files>src/shared/components/TaskForm.tsx, src/app/(app)/tasks/new/page.tsx</files>
  <action>
...
```
  ...
  <verify>Run `npm run dev` and visit /tasks/new - form renders with tag input</verify>
  <done>Task creation form includes tag input, tags are added after task creation</done>
</task>

<task type="auto">
  <name>Task 5: Update TaskEditForm to include tag management</name>
  <files>src/shared/components/TaskEditForm.tsx, src/app/(app)/tasks/[id]/edit/page.tsx</files>
  <action>
...
3. Export the new component from the components index.
  ...
  <verify>Run `npm run dev` and visit /tasks/[id]/edit - form renders with current tags</verify>
  <done>Task edit form allows adding/removing tags with immediate server sync</done>
</task>
...
<output>
After completion, create `.planning/phases/04-task-organization-discovery/04-03-SUMMARY.md`
</output>
