...
<objective>
Create GitHub API integration with proper rate limit handling for user enrichment data.

Purpose: Enable fetching additional user data from GitHub (repos, activity patterns) for brain learning while respecting API rate limits and preventing service disruptions. This builds on the existing GitHub OAuth integration from Phase 2.

Output: GitHub API client with ETag caching, rate limit monitoring, exponential backoff, and graceful degradation when limits are hit.
</objective>
...
<task type="auto">
  ...
  <action>
Create `src/shared/lib/analytics/github.ts` with comprehensive rate limit handling:
...
CRITICAL: Always include timeout with AbortSignal.timeout() to prevent hanging requests.
...
- Functions can be imported from the module
  ...
  <done>GitHub API client with rate limiting, ETag caching, and graceful degradation</done>
</task>
...
Note: If Plan 01 hasn't run yet (no tracker.ts), just export the GitHub functions. The tracker exports will be added by Plan 01.
...
<task type="auto">
  ...
  <action>
Create a simple test file to verify the GitHub integration works correctly. This can be run manually or via test runner.
...
AVOID: Writing tests that require external API calls during CI - mock data for automated tests.
  </action>
  <verify>
- Test file compiles without TypeScript errors
- `npx tsx src/shared/lib/analytics/__tests__/github.test.ts` runs without crashing
- Graceful degradation test passes (returns null for nonexistent user)
  </verify>
  <done>Integration test verifies GitHub client handles edge cases gracefully</done>
</task>
...
<output>
After completion, create `.planning/phases/06-github-integration-analytics/06-02-SUMMARY.md`
</output>
