...
**Domain:** Next.js 15 performance optimization, Core Web Vitals, responsive design
**Confidence:** HIGH
...
- Bundle analysis with `@next/bundle-analyzer` identifies optimization opportunities; the project should target <100KB initial JavaScript for fast loads

**Primary recommendation:** Use Next.js built-in features first (Server Components, loading.js, Image component), then add progressive enhancements (react-swipeable for mobile, bundle splitting for heavy features like file management). Monitor with Lighthouse CI and real user monitoring.
...
| `@next/bundle-analyzer` | Latest | Webpack bundle visualization | Official Next.js plugin, Turbopack support in v16+ |
| `react-swipeable` | 7.0.2 | Mobile touch gestures | Minimal (useSwipeable hook), versatile, works on any element |
...
| `web-vitals` | Latest | Core Web Vitals measurement | Real user monitoring via `useReportWebVitals` hook |
...
| `@next/bundle-analyzer` | `webpack-bundle-analyzer` directly | Same tool underneath, but @next/bundle-analyzer is pre-configured |
...
- **Don't lazy load entire pages** — Only lazy load specific heavy components, not whole page modules
- **Don't use `React.lazy()` in Next.js** — Use `next/dynamic` for SSR compatibility
- **Don't set `priority` on multiple images** — Only mark true LCP element (or use `preload` in v16)
- **Don't skip width/height on images** — Causes layout shift (CLS penalty)
- **Don't lazy load above-the-fold content** — Hurts LCP; only lazy load below the fold
- **Don't use custom loading without testing** — Over-lazy loading increases latency
...
|---------|-------------|-------------|-----|
| Mobile swipe gestures | Custom touch event handlers | `react-swipeable` | Handles edge cases: touch vs mouse, scroll prevention, delta thresholds, cross-browser compatibility |
| Image optimization | Manual WebP conversion | Next.js `<Image>` component | Automatic format selection, responsive srcset, lazy loading, blur placeholders, prevents CLS |
| Bundle analysis | Manual webpack stats parsing | `@next/bundle-analyzer` | Pre-configured for Next.js, visual treemap, Turbopack support |
| Lazy loading | Custom intersection observers | `next/dynamic` | SSR-compatible, built-in Suspense, automatic code splitting |
| Skeleton screens | Loading spinners everywhere | Next.js `loading.js` | Route-level convention, automatic Suspense wrapping, streaming support |
| Core Web Vitals tracking | Custom performance.now() logic | `web-vitals` library + `useReportWebVitals` | Google's official library, matches Search Console data, 2KB size |

**Key insight:** Next.js has already solved most performance problems through built-in features. The framework's defaults (Server Components, automatic code splitting, Image component) handle 80% of optimization work. Custom solutions introduce bugs and maintenance burden.
...
|--------------|------------------|--------------|--------|
| FID (First Input Delay) metric | INP (Interaction to Next Paint) | March 2024 | INP is stricter (200ms vs 100ms), measures all interactions not just first |
| `priority` prop on Image | `preload` prop | Next.js 16 (2025) | Same functionality, clearer naming |
| Client-side rendering for everything | Server Components by default | Next.js 13 (2022) | Massive bundle reduction, faster initial loads |
| Manual lazy loading with React.lazy() | next/dynamic with SSR support | Always (Next.js convention) | SSR compatibility, better DX |
| Custom loading spinners | loading.js convention | Next.js 13 (2022) | Automatic Suspense boundaries, streaming support |
| Manual route prefetching | Automatic on Link hover | Next.js 10+ | Instant navigation feel |
...
1. **Performance Budget Thresholds**
   - What we know: Lighthouse CI can enforce budgets, typical targets are <100KB JS for initial load
   - What's unclear: Specific bundle size limits for this project (depends on features)
   - Recommendation: Run bundle analyzer first, then set realistic budgets based on current state

2. **Real User Monitoring vs Synthetic Testing**
   - What we know: RUM (Real User Monitoring) tracks actual users, synthetic tests (Lighthouse) are lab-based
   - What's unclear: Best balance for this project phase (RUM requires user traffic)
   - Recommendation: Start with Lighthouse CI (synthetic), add RUM in Phase 8 (Deployment) when users exist
...
3. **Mobile Browser Testing Coverage**
   ...
   - Recommendation: Test latest iOS + Android, use BrowserStack for older versions if needed

4. **Progressive Web App (PWA) Features**
   - What we know: PWA adds offline support, install prompts, service workers
   - What's unclear: Whether PWA is in scope for this phase (not mentioned in requirements)
...
- [Next.js Lazy Loading](https://nextjs.org/docs/app/guides/lazy-loading) - next/dynamic usage, SSR options, pitfalls
- [Next.js Image Component](https://nextjs.org/docs/app/api-reference/components/image) - Automatic optimization, lazy loading, responsive images
- [Next.js Package Bundling](https://nextjs.org/docs/app/guides/package-bundling) - optimizePackageImports, serverExternalPackages, bundle reduction
- [Google Core Web Vitals](https://developers.google.com/search/docs/appearance/core-web-vitals) - LCP, INP, CLS metrics and thresholds
- [Tailwind Responsive Design](https://tailwindcss.com/docs/responsive-design) - Mobile-first breakpoint system
...
- Mobile gestures: MEDIUM - react-swipeable is well-documented but implementation details need testing
- Performance budgets: LOW - Project-specific thresholds require bundle analysis to determine

**Research date:** 2026-01-26
**Valid until:** 60 days (Next.js stable, stack mature, Core Web Vitals unlikely to change)

**Note on INP vs FID:** Google officially replaced FID with INP in March 2024. All references to Core Web Vitals should use INP (target <200ms), not FID.
