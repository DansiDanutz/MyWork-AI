...
Purpose: Reduce initial bundle size by deferring load of file upload/management components. FileDropzone includes tus-js-client, react-dropzone, and file-type - these should only load when editing tasks with file attachments.

Output: Lazy-loaded wrappers for file components, updated imports in TaskEditFormWithTags
</objective>
...
Note: Need to also export FileDropzoneProps from FileDropzone.tsx if not already exported.
...
- `npx tsc --noEmit`
  ...
  <done>
LazyFileDropzone and LazyFileList wrappers exist using next/dynamic with proper loading fallbacks and ssr: false
  </done>
</task>
...
1. In `TaskEditFormWithTags.tsx`:
   - Replace `import { FileDropzone } from './FileDropzone'` with `import { LazyFileDropzone } from './LazyFileDropzone'`
   - Replace `import { FileList } from './FileList'` with `import { LazyFileList } from './LazyFileList'`
...
- TypeScript compiles: `npx tsc --noEmit`
  ...
  <done>
TaskEditFormWithTags uses lazy-loaded file components, reducing initial bundle size for task edit page
  </done>
</task>
...
<success_criteria>
- Heavy file components (FileDropzone, FileList) load lazily with visible loading states
...
<output>
After completion, create `.planning/phases/07-performance-quality/07-02-SUMMARY.md`
</output>
