#!/usr/bin/env python3
"""
Autocoder API Integration Tool
==============================
Provides programmatic control over Autocoder with both automatic and manual modes.

Autocoder Server: http://127.0.0.1:8888
WebSocket: ws://127.0.0.1:8888/ws/projects/{project_name}

Usage:
    # Check if Autocoder server is running
...
class AutocoderAPI:
    ...
    def __init__(self, base_url: str = AUTOCODER_API):
    ...
    def start_agent(
        ...
        try:
            ...
            return {"error": "Server not running. Start with: python tools/autocoder_api.py server"}
        except Exception as e:
            return {"error": str(e)}
...
def start_server():
    """Start the Autocoder server."""
    api = AutocoderAPI()
    if api.is_server_running():
        print("Autocoder server is already running at http://127.0.0.1:8888")
        return True

    print("Starting Autocoder server...")
...
def open_ui():
    """Open the Autocoder UI in browser."""
    api = AutocoderAPI()
    ...
    print("Opened Autocoder UI in browser")
...
def main():
    parser = argparse.ArgumentParser(
        description="Autocoder API Integration Tool",
...
"""
    ...
    # Status command
    subparsers.add_parser("status", help="Check if Autocoder server is running")

    # Server command
    subparsers.add_parser("server", help="Start Autocoder server")

    # UI command
    subparsers.add_parser("ui", help="Open Autocoder UI in browser (manual mode)")
    ...
    if args.command == "status":
        if api.is_server_running():
            print("Autocoder server is RUNNING at http://127.0.0.1:8888")
        else:
            print("Autocoder server is NOT RUNNING")
            print("Start with: python tools/autocoder_api.py server")
