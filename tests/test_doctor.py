...
def test_doctor_runs():
    ...
    r = subprocess.run([sys.executable, "tools/mw.py", "doctor"], capture_output=True, text=True, timeout=45)
...
def test_doctor_alias():
    ...
    r = subprocess.run([sys.executable, "tools/mw.py", "diagnose"], capture_output=True, text=True, timeout=45)
    assert "Doctor" in r.stdout or "DIAGNOSTIC" in r.stdout or "HEALTH" in r.stdout
